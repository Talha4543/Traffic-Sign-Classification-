...
def classify_image(image):
    # Resize the image to 30x30 pixels as per the model's expected input size
    ...
    pred_probability = np.max(pred, axis=1)[0]  # Get the highest probability
    
    # Set a confidence threshold (e.g., 50%)
    if pred_probability < 0.5:
        return "Invalid image (not a traffic sign)."
    
    # Return the class name if confidence is sufficient
    return classes[pred_class + 1]
